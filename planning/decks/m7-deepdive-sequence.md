...
| 17 | Judge the marker strictly, judge the prose by shape | The comparison table's honest-reading rule, applied across baseline/A/B: `OUTCOME:` and which stages ran are exact reproducible facts; wording is small-model noise unless the deck's own note says otherwise | Numbered rows: baseline (blue, APPROVED, 5 calls) → Variant A (orange, APPROVED ×3, prose drifted) → Variant B (red, APPROVED — but baseline correctly REJECTED this incident, "not instability — a clean reproducible failure") | Variant B isn't noise like Variant A — it's a deterministic consequence of removing a gate, reproducible every time you run it with the gate off |
...
"outcome stable, prose drifts" reading, 16 for the "clean reproducible failure, not noise"
reading, because conflating those two readings is the exact honesty mistake the module's `:::note`
warns against — and 17 closes the loop with the comparison table's judging rule. Under time
...
| §6 — comparison table's judging rule: marker is exact and reproducible, prose is shape-only, Variant B is not noise | 17 | Numbered rows: baseline/A/B summarized, B flagged "not instability — a clean reproducible failure" |
